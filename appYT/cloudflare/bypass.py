...
DEFAULT_USER_AGENT = 'Mozilla/5.0 (X11; Linux i686; rv:45.0) Gecko/20100101 Firefox/45.0'
...
class CloudflareScraper(Session):
    ...
    def solve_cf_challenge(self, resp, **kwargs):
        ...
        try:
            params["jschl_vc"] = search(r'name="jschl_vc" value="(\w+)"', body).group(1)
            params["pass"] = search(r'name="pass" value="(.+?)"', body).group(1)
    ...
    def extract_js(self, body):
        js = search(r"setTimeout\(function\(\){\s+(var "
                        "s,t,o,p,b,r,e,a,k,i,n,g,f.+?\r?\n[\s\S]+?a\.value =.+?)\r?\n", body).group(1)
        js = sub(r"a\.value = (parseInt\(.+?\)).+", r"\1", js)
        js = sub(r"\s{3,}[a-z](?: = |\.).+", "", js)
...
# print get_tokens('http://www.animeyt.tv/')
# print get_cookie_string('http://www.animeyt.tv/',user_agent='Mozilla/5.0 (X11; Linux i686; rv:45.0) Gecko/20100101 Firefox/45.0')[0]
# print CloudflareScraper().request('get','http://www.animeyt.tv/').content